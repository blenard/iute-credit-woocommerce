var EshopCalculator = function(){
    ...
    this.findAmountLimitedPeriodParameterSets = function (periodParameters) {
        ...
        for (var index in periodParameters) {
            ...
            if($('.iute-calc-single').length){
                periodParameterSet.minAmount = 0;
            }
    ...
    this.getCurrentPeriodSelectionValue = function() {
        let single = $('.iute-calc-single-range');
        if(single.length){
            console.log(single.val());
            return parseInt(single.val());
        } else {
    ...
    this.updateResultsView = function(result) {
        ...
        let single = $('.iute-calc-single-range');
        if(single.length){
            $('.iute-calc-single time span').text(single.val());
            $('.iute-loan__price label').text(this.addCommas(result.monthlyPayment));
        }
    };
    ...
    this.addProductTabs = function(calculationParameterSet, index, showNameTab) {
        ...
        if($('input[name="iute_appl_pr_id"]').length){
            if (!$('input[name="iute_appl_pr_id"]').val()) {
