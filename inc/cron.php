...
{ 
	...
	foreach ($orders as $key => $post_order) {
		...
		require_once 'class-wc-iute-mail.php';
		$mailer = new WC_Iute_Status_Email();

		$iute = new WC_Gateway_IuteCredit;

		$send_to_admin = $iute->get_option('email_admin');
		$send_to_user = $order->billing_email;

		if ($status->status != 'IN_PROCESS') {
			// send email to admin and client
			$mailer->send_mail(array($send_to_admin, $send_to_user), require_once plugin_dir_path( __DIR__ ) . 'templates/template-email-html.php');
		}
	}
}