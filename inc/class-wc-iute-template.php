...
class WC_Gateway_Iute_Template
{
	public function add_to_cart_before ($username, $password, $agent_id, $amount)
	{
		...
		$base_loan = $base_loan_months->monthlyRepayments[sizeof($base_loan_months->monthlyRepayments) - 1]->monthlyRepayment;
		return '<div class="iute-loan"><span>' . __( 'Monthly payment', 'iutecredit-payment' ) . '</span><span class="iute-loan__price">' . $base_loan . get_woocommerce_currency_symbol() . '</span></div>';
	}
